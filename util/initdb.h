...
QSqlError initDb()
{
    //QSqlQueryModel: class provides a read only data model for SQL result set
    QSqlDatabase db = QSqlDatabase::addDatabase("QMYSQL");
    //bool ok = false;
    //fprintf(stderr, "%s Koneksi db: %d\r\n", __FUNCTION__, ok) ;
    db.setHostName("localhost");
    db.setDatabaseName("ws");
    db.setUserName("monita");
    db.setPassword("monita2011");
    //ok = db.open();
    //fprintf(stderr, "Koneksi db: %d\r\n", ok) ;
